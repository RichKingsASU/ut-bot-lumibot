...
| I-01 | I — Supply chain | Python/container dependencies are not reproducibly pinned; infrastructure images use `latest`; scorer/vector containers install floating packages at every start. | HIGH | FAIL | Y | Recreate can drift, fail dependency resolution, or crash-loop while old containers looked healthy. | `requirements.txt` uses `>=` or no version; `docker-compose.yml:25,35,42,49-50,64-65,78-104`; sentiment/vector commands install unpinned packages at runtime. |
